...
/*Fragment del menú de game over del minijuego 1*/
public class ShooterGameOverFragment extends BaseFragment {

    private int score;
    private boolean victory;

    public ShooterGameOverFragment(){}

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceBundle){
        View root = inflater.inflate(R.layout.fragment_shooter_game_over, container, false);
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        Button buttonBack = (Button) view.findViewById(R.id.buttonRetryShooter);
    ...
    //Establece los récords
    private void setupLeaderboard(){
        int top1 = getMainActivity().getStatsTracker().getTop1_record_game1();
        int top2 = getMainActivity().getStatsTracker().getTop2_record_game1();
        int top3 = getMainActivity().getStatsTracker().getTop3_record_game1();

        if (score > top1){
            getMainActivity().getStatsTracker().setTop1_record_game1(score);
            getMainActivity().getStatsTracker().setTop2_record_game1(top2);
            getMainActivity().getStatsTracker().setTop3_record_game1(top3);
            getMainActivity().toastMessage("¡Nuevo récord!");
        } else if (score > top2){
            getMainActivity().getStatsTracker().setTop2_record_game1(score);
            getMainActivity().getStatsTracker().setTop3_record_game1(top2);
            getMainActivity().toastMessage("¡Nuevo récord!");
        } else if (score > top3){
            getMainActivity().getStatsTracker().setTop3_record_game1(score);
            getMainActivity().toastMessage("¡Nuevo récord!");
        } else {
